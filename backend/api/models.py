...
class Product(Updateable, BaseModel):
    ...
    _days_left: so.Mapped[int] = so.mapped_column(sa.Integer, nullable=True)
    expire: so.Mapped[Optional[bool]] = so.mapped_column(sa.Boolean, default=False)
    ...
    @days_left.setter
    def days_left(self, value: int):
        if not isinstance(value, int):
            raise ValueError("Days left must be a number")

        self._days_left = value

        # Set expire only when days_left is 0
        if value == 0:
            self.expire = True

    def update_expiration_status(self):
        """Check and update expiration status based on deadline countdown."""
        if self.days_left == 0:
            self.expire = True
...
class Cart(BaseModel):
    ...
    size: so.Mapped[str] = so.mapped_column(sa.String(20), nullable=False)
    total_price: so.Mapped[Optional[int]] = so.mapped_column(sa.Integer, default=0)
