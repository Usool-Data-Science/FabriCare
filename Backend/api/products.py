...
@products_bp.route('/media/<path:filename>')
...
def new_sweater(data):
    ...
    if main_image.content_length > current_app.config['MAX_FILE_SIZE']:
    ...
    for img in sub_images:
        ...
        if img.content_length > current_app.config['MAX_FILE_SIZE']:
    ...
    main_image_path = os.path.join(current_app.config['MEDIA_PATH'], main_image_filename)
    ...
    try:
        ...
        data['mainImage'] = main_image_path
        data['subImages'] = sub_image_paths
        # data['subImages'] = [os.path.relpath(path, current_app.config['MEDIA_PATH']) for path in sub_image_paths]
...
def update_product(data, id):
    ...
    if new_main_image:
        ...
        if not allowed_file(new_main_image):
            abort(400, description="Invalid mainImage file type.")
        if new_main_image.content_length > current_app.config['MAX_FILE_SIZE']:
            abort(400, description="Main image exceeds size limit.")
    ...
    try:
        ...
        if new_main_image:
            ...
            old_main_image_path = os.path.join(current_app.config['MEDIA_PATH'], product.mainImage)
        ...
        if new_sub_images:
            ...
            old_sub_image_paths = [
                os.path.join(current_app.config['MEDIA_PATH'], img) for img in product.subImages
            ]
